...
fastapi-mvc available under: http://leds-cibele-api.192.168.49.2.nip.io/
...
...
Kubernetes cluster ready
FastAPI available under: http://leds-cibele-api.192.168.49.2.nip.io/
You can delete dev-env by issuing: make clean
vagrant@ubuntu-focal:/syncd$ kubectl get all -n leds-cibele-api
NAME                                                     READY   STATUS    RESTARTS   AGE
pod/leds-cibele-api-7f4dd8dc7f-p2kr7                1/1     Running   0          55s
...
service/leds-cibele-api                ClusterIP   10.110.42.252   <none>        8000/TCP    56s
service/rfs-redisfailover-persistent-keep   ClusterIP   10.110.4.24     <none>        26379/TCP   3m39s

NAME                                                READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/leds-cibele-api                1/1     1            1           55s
deployment.apps/rfs-redisfailover-persistent-keep   3/3     3            3           3m39s

NAME                                                           DESIRED   CURRENT   READY   AGE
replicaset.apps/leds-cibele-api-7f4dd8dc7f                1         1         1       55s
...
vagrant@ubuntu-focal:/syncd$ curl http://leds-cibele-api.192.168.49.2.nip.io/api/ready
{"status":"ok"}
```
...
To bootstrap local minikube Kubernetes cluster exposing `leds-cibele-api` application:
```shell
make dev-env
```
...
```shell
$ leds-cibele-api --help
Usage: leds-cibele-api [OPTIONS] COMMAND [ARGS]...

  Leds-cibele-api CLI root.
...
Commands:
  serve  leds-cibele-api CLI serve command.
$ leds-cibele-api serve --help
Usage: leds-cibele-api serve [OPTIONS]
...
```shell
$ make image
$ podman run -dit --name leds-cibele-api -p 8000:8000 leds-cibele-api:$(cat TAG)
f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
$ podman ps
CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
f41e5fa7ffd5  localhost/leds-cibele-api:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  leds-cibele-api
$ curl localhost:8000/api/ready
{"status":"ok"}
```
...
All application configuration is available in `leds_cibele_api.config` submodule.
...
| FASTAPI_PROJECT_NAME | `"leds-cibele-api"`                               | FastAPI project name.                                          |
...
1. Source: `leds_cibele_api.config/gunicorn.conf.py`
2. [Gunicorn configuration file documentation](https://docs.gunicorn.org/en/latest/settings.html)

### Routes definition

Endpoints are defined in `leds_cibele_api.app.router`. Just simply import your controller and include it to FastAPI router:

```python
from fastapi import APIRouter
from leds_cibele_api.app.controllers.api.v1 import ready
...
You can implement your own web routes logic straight away in `leds_cibele_api.app.controllers.api.v1` submodule. For more information please see [FastAPI documentation](https://fastapi.tiangolo.com/tutorial/).

### Utilities

For your discretion, I've provided some basic utilities:
* RedisClient `leds_cibele_api.app.utils.redis`
* AiohttpClient `leds_cibele_api.app.utils.aiohttp_client`
...
Example:
```python
from leds_cibele_api.app.utils import RedisClient

response = RedisClient.get("Key")
```
```python
from leds_cibele_api.app.utils import AiohttpClient
...
Source: `leds_cibele_api.app.exceptions.http.py`
